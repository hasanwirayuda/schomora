...
export default function HomePage() {
  return (
    <main className="relative min-h-screen w-full flex items-center justify-center bg-[#6520BE]">
      <div
        className="absolute inset-0 bg-[url('/background.svg')] bg-cover bg-center"
        aria-hidden="true"
      />
      ...
      <section className="relative flex flex-col items-center gap-8">
        ...
        <div className="flex flex-col gap-2 p-4 border border-white/20 rounded-md bg-white/2 backdrop-blur-xs text-center">
          <Link
            href="/register"
            className="px-10 py-2.5 rounded-md bg-[#23AFB6] text-white text-lg hover:bg-[#34c7d2] transition-colors border border-white/10 shadow-md/50"
          >
            Get Started
          </Link>
          ...
          <Link
            href="/login"
            className="px-10 py-2.5 border-2 border-white/50 rounded-md text-lg bg-transparent text-white hover:bg-white/15 transition-all"
          >
